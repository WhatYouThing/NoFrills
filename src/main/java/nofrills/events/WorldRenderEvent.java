...
public class WorldRenderEvent {
    ...
    public void drawFilled(AABB box, boolean throughWalls, RenderColor color) {
        this.storage.submitShapeOutline(this.matrices, Shapes.create(box), RenderTypes.debugFilledBox(), color.argb, 3.0f, throughWalls);
    }

    public void drawOutline(AABB box, boolean throughWalls, RenderColor color) {
        this.storage.submitShapeOutline(this.matrices, Shapes.create(box), RenderTypes.linesTranslucent(), color.argb, 3.0f, throughWalls);
    }
    ...
    public void drawText(Vec3 pos, Component text, float scale, boolean throughWalls, RenderColor color) {
        ...
        this.storage.submitText(matrices, -mc.font.width(text) / 2f, 1.0f, text.getVisualOrderText(), true, throughWalls ? Font.DisplayMode.SEE_THROUGH : Font.DisplayMode.NORMAL, FULL_BRIGHT, color.argb, -1, -1);
    }
