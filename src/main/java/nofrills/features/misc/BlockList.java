...
public class BlockList {
    ...
    public static MutableComponent buildEntryLine(JsonObject entry, String text) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(entry.get("timestamp").getAsLong());
        MutableComponent tooltip = Component.literal(Utils.format("§7Blocked reason: {}\n§7Blocked date: {}",
                entry.get("reason").getAsString(),
                Utils.parseDate(calendar)
        ));
        return Component.literal(text).setStyle(Style.EMPTY.withHoverEvent(new HoverEvent.ShowText(tooltip)));
    }
        ...
        if (instance.isActive() && autoKick.value() && event.msg().startsWith("Party Finder >") && event.msg().contains("joined")) {
            ...
            forEntry(name, (obj) -> {
                if (obj.isPresent()) {
                    ...
                    MutableComponent message = Component.literal("§cAutomatically kicking blocked player ")
                            .append(Component.literal(name).setStyle(style.orElse(Style.EMPTY.applyFormat(ChatFormatting.GRAY)))).append("§c.")
